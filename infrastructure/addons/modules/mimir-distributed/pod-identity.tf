...
# EKS Pod Identity for Mimir to assume IAM role
module "mimir_pod_identity" {
  ...
  source_policy_documents = [data.aws_iam_policy_document.mimir_s3_access.json]

  # Association to the mimir-distributed service account
  associations = {
    mimir = {
